...
from .overlay import OverlayManager, TextOverlayParser
from .utils import *
from .grouping_scheme_access import AccessDescIdAttrMixin, GroupingSchemeAccess
from .box import UVPM3_Box
from .box_utils import BoxRenderInfo, BoxRenderer, BoxRenderAccess, CustomTargetBoxAccess, disable_box_rendering
...
class RenderBoxesOverlayManager(OverlayManager):
    ...
    SNAP_PROMPT_MSG = 'Hold CTRL to snap to tile or other box boundaries'
    ...
    def __init__(self, op, context):
        ...
        self.mode_to_msg_array = {
            ...
            RenderBoxMode.MOVE : self.move_overlapy_msg_array
        }
    ...
    def move_overlapy_msg_array(self):
        msg_array = []
        msg_array.append(self.MOVE_HEADER_MSG)
        msg_array.append(self.SNAP_PROMPT_MSG)
        msg_array.append(self.CANCEL_PROMPT_MSG)
        return msg_array
...
class UVPM3_OT_RenderBoxesGeneric(bpy.types.Operator):
    ...
    def apply_snapping(self, points, mask=(True, True, True, True), keep_dimensions=False, self_points_multiples=False):
        ...
        snap_results = [(0.0, self.SNAP_DISTANCE)]*4
        for snap_points in snap_points_list:
            for idx, snap_point in enumerate(snap_points):
                ...
                for check_point in check_points:
                    delta = check_point - points[idx]
                    distance = abs(delta)
                    if distance < snap_results[idx][1]:
                        snap_results[idx] = (delta, distance)

        if self_points_multiples:
            snap_points = self.active_box_stored.coords_tuple()
            ...
            for idx, snap_point in enumerate(snap_points):
                ...
                check_points = (snap_point, snap_points[(idx+2) % len(snap_points)])
                segment = max(check_points) - min(check_points)
                for check_point in check_points:
                    if check_point == 0.0 or check_point % 1 == 0:
                        continue
                    grid_offset = check_point % segment
                    grid = grid_offset + (math.floor((points[idx] + segment/2) / segment) * segment)
                    delta = grid - points[idx]
                    distance = abs(delta)
                    if distance < snap_results[idx][1]:
                        snap_results[idx] = (delta, distance)

        if keep_dimensions:
            delta_x = snap_results[0][0] if snap_results[0][1] < snap_results[2][1] else snap_results[2][0]
            delta_y = snap_results[1][0] if snap_results[1][1] < snap_results[3][1] else snap_results[3][0]
            points[0] += delta_x
            points[1] += delta_y
            points[2] += delta_x
            points[3] += delta_y
            ...
            points[0] += snap_results[0][0]
            points[1] += snap_results[1][0]
            points[2] += snap_results[2][0]
            points[3] += snap_results[3][0]

        return points

    def handle_draw(self, event, view_coords):
        ...
        if self.draw_begin is not None and self.draw_end is not None:
            ...
            self.active_box_info.box = UVPM3_Box.SA(*draw_coords)
    ...
    def handle_box_selection(self, event, view_coords):
    ...
    def handle_resize(self, event, view_coords):
        ...
        if self.render_mode == RenderBoxMode.RESIZE:
            ...
            if event.type == 'LEFTMOUSE' and event.value == 'RELEASE':
                ...
                self.active_box_info.box = UVPM3_Box.SA(*new_coords)
                self.prefs.boxes_dirty = True
            return
    ...
    def handle_move(self, event, view_coords):

        if self.render_mode not in (RenderBoxMode.IDLE, RenderBoxMode.MOVE):
            return
        ...
        if self.render_mode == RenderBoxMode.MOVE:
            if event.type == 'LEFTMOUSE' and event.value == 'RELEASE':
                self.appy_box_transformation(drawn_box)
                self.force_block_event = False
                self.prefs.boxes_dirty = True
                ...
                new_coords[0] = view_coords[0] + self.move_delta_coords[0]
                new_coords[1] = view_coords[1] + self.move_delta_coords[1]
                new_coords[2] = view_coords[0] + self.move_delta_coords[2]
                new_coords[3] = view_coords[1] + self.move_delta_coords[3]

                if self.snap_enable:
                   new_coords = self.apply_snapping(new_coords, keep_dimensions=True, self_points_multiples=True)

                self.active_box_info.box = UVPM3_Box.SA(*new_coords)
                self.prefs.boxes_dirty = True
            return

        is_inside = self.active_box_info.box.point_inside(view_coords)

        if is_inside:
            self.cursor_id = "HAND"

        if is_inside and event.type == 'LEFTMOUSE' and event.value == 'PRESS':
            self.render_mode = RenderBoxMode.MOVE
            self.move_delta_coords = (drawn_box.p1_x-view_coords[0], drawn_box.p1_y-view_coords[1],
                                      drawn_box.p2_x-view_coords[0], drawn_box.p2_y-view_coords[1])
            self.force_block_event = True
            self.active_box_stored = self.active_box_info.box
    ...
    def update_text(self, box_info):
        ...
        if box_info is self.active_box_info:
            ...
            box_info.text = TextOverlayParser.parse_text("<t color='{color}'>[</t> {text} <t color='{color}'>]</t>".format(text=box_info.text_raw, color=outline_color))
    ...
    def modal(self, context, event):
        ...
        try:
            ...
            if event.type != 'TIMER':
                ...
                self.handle_box_selection(event, view_coords)
                self.handle_resize(event, view_coords)
                self.handle_move(event, view_coords)
                self.handle_draw(event, view_coords)
...
class ActiveGroupingSchemeRenderAccess(GroupingSchemeRenderAccess, GroupingSchemeAccess):
    ...
    def __init__(self, access_desc_id):
        self.access_desc_id = access_desc_id

    def init_access(self, context):
        GroupingSchemeAccess.init_access(self, context, desc_id=self.access_desc_id)
...
class UVPM3_OT_RenderGroupingSchemeBoxes(UVPM3_OT_RenderBoxesGeneric, AccessDescIdAttrMixin):
    ...
    def get_box_access(self):
        ...
        return ActiveGroupingSchemeRenderAccess(access_desc_id=self.access_desc_id)
...
class UVPM3_OT_SetGroupingSchemeBoxToTile(GroupingSchemeAccess, UVPM3_OT_SetTargetBoxToTile, AccessDescIdAttrMixin):
...
class UVPM3_OT_MoveGroupingSchemeBox(GroupingSchemeAccess, UVPM3_OT_MoveTargetBox, MoveTargetBoxProperties, AccessDescIdAttrMixin):
...
class UVPM3_OT_SelectIslandsInGroupingSchemeBox(GroupingSchemeAccess, UVPM3_OT_SelectIslandsInBox, SelectIslandsInBoxProperties, AccessDescIdAttrMixin):
