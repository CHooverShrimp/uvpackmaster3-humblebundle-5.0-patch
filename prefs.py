...
from .island_params import ScaleLimitIParamInfo, AlignPriorityIParamInfo, NormalizeMultiplierIParamInfo
...
class UVPM3_NumberedGroupsDescriptor(bpy.types.PropertyGroup):
...
class UVPM3_NumberedGroupsDescriptorContainer(bpy.types.PropertyGroup):
...
class UVPM3_Preferences(AddonPreferences):
    ...
    def fixed_scale_not_supported_msg(self, scene_props):
        return None

    def fixed_scale_enabled(self, scene_props):
        return (self.fixed_scale_not_supported_msg(scene_props) is None) and scene_props.fixed_scale

    def normalize_scale_not_supported_msg(self, scene_props):
        if self.fixed_scale_enabled(scene_props):
            return "Not supported with 'Fixed Scale' enabled"
    ...
    def tile_filling_method_not_supported_msg(self, scene_props):
        if self.fixed_scale_enabled(scene_props):
            return "When 'Fixed Scale' is enabled, the '{}' method is always used".format(Labels.TILE_FILLING_METHOD_ONE_BY_ONE_NAME)
    ...
    def get_g_schemes_preset_path(self):
