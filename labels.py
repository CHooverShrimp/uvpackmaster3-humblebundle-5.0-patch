...
class Labels:
    ...
    NORMALIZE_SCALE_DESC='Automatically scale selected islands before packing so that the average texel density is the same for every island. WARNING: if lock overlapping is enabled, then normalization takes place AFTER overlapping islands are locked'
    ...
    HEURISTIC_ALLOW_MIXED_SCALES_DESC="Allow the packer to apply inconsistent scaling to islands during the search in order to improve UV space coverage. For example, when a single island in the UV map is much bigger than all other islands, such an island may limit final UV space coverage: the big island results in an upper limit for scale (because it has to be fit into the target box) and that limit also applies for all other islands when uniform scaling is used. When the '{}' option is enabled, the packer is allowed to scale the big island down relatively to other islands - meaning the upper limit for scale no longer applies to the smaller islands. In result the smaller islands may be scaled up resulting in better overall UV space coverage. WARNING: when the option is enabled, the packer may scale islands independently. It means the relative texel density of islands may be changed after packing".format(HEURISTIC_ALLOW_MIXED_SCALES_NAME)
    ...
    SIMI_NON_UNIFORM_SCALING_TOLERANCE_DESC='Determines to what extent the packer will be allowed to apply non-uniform scaling when determining if two islands are similar. If set to 0, non-uniform scaling will not be allowed at all. If the value is between 0 and 1, the packer will consider matches resulting from non-uniform scaling, but it will add a penalty for such matches when comparing them with uniformly scaled matches. A general rule is: the more non-uniformly a match is scaled, the greater the penalty which will be applied for the match during comparison. On the other hand the greater the value this option, the more tolerance the packer will have for non-uniform scaling i.e. overall penalties for non-uniformly scaled matches will be lower. If the option is set to the maximum tolerance (value 1), the packer will not distinguish non-uniform scaling from uniform scaling (both cases will be preferred equally)'
