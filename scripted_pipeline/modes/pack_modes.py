...
from ...island_params import NormalizeMultiplierIParamInfo, ScaleLimitIParamInfo, VColorIParamSerializer, RotStepIParamInfo
...
class UVPM3_PT_OverrideGlobalOptionsPopover(bpy.types.Panel, PanelUtilsMixin, PresetPanel):
    ...
    def draw(self, context):
        ...
        row = ow_col.row(align=True)
        row.prop(overrides, "override_pack_to_single_box", text='')
        box_s = row.box()
        box_s.prop(overrides, "pack_to_single_box")
        box_s.enabled = overrides.override_pack_to_single_box
