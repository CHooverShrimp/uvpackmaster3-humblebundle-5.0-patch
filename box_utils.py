...
SHADER_NAME_FLAT_COLOR = 'FLAT_COLOR' if bpy.app.version >= (4, 0, 0) else '3D_FLAT_COLOR'
...
class BoxRenderInfo:
    ...
    def __init__(self, glob_idx, box, color, text, text_line_num=0, z_coord=0.0):
        ...
        self.text = TextOverlayParser.parse_text(text)
        self.text_raw = text
...
class BoxRenderer(OverlayManager):
    ...
    def update_coords(self):
        ...
        def append_line_to_batch(p1, p2, fixed_coord, offset, z_coord, color):
            ...
            append_box_to_batch(UVPM3_Box.SA(p1[0], p1[1], p2[0], p2[1]), z_coord, color)
...
class CustomTargetBoxAccess(BoxArrayRenderAccess):
    ...
    def init_access(self, context, ui_drawing=False):
        ...
        return super().init_access([get_scene_props(context).custom_target_box], self.MAIN_TARGET_BOX_COLOR, active_idx=0)
