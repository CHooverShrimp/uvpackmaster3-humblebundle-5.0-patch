...
def rna_callback():
    disable_box_rendering(None, bpy.context)


def rna_callback_subscribe():
    handle = object()
...
def rna_callback_unsubscribe(handle):
...
def load_post_handler(scene):
    ...
    # get_prefs()._rna_callback_handle = rna_callback_subscribe()
    return None
...
def register_specific():
    ...
    addon_engine_path = os.path.join(os.path.dirname(os.path.abspath(process_file_path(__file__))), 'engine3')
